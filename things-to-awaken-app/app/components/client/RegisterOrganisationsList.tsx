"use client";

import {
  Table,
  TableBody,
  TableCaption,
  TableCell,
  TableFooter,
  TableHead,
  TableHeader,
  TableRow,
} from "@/components/ui/table";
import { useToast } from "@/components/ui/use-toast";
import { OrganisationDto } from "@/lib/types";
import Link from "next/link";
import { SyntheticEvent, useState } from "react";
import GenericDialog from "../shared/GenericDialog";
import { RegisterDonationToOrganisationForm } from "./RegisterDonationBox";
import useDonationStore from "@/app/store/donationStore";

export function RegisterOrganisationsList({
  orgs,
}: {
  orgs: OrganisationDto[];
}) {
  const [showEditOrgDialog, setShowEditOrgDialog] = useState(false);
  const [selectedOrg, setSelectedOrg] = useState<OrganisationDto | null>();

  const handleToggleDialog = (
    org: OrganisationDto | null,
    event: SyntheticEvent<EventTarget> | boolean
  ) => {
    setSelectedOrg(org);
    if (typeof event !== "boolean") event.stopPropagation();
    setShowEditOrgDialog(!showEditOrgDialog);
  };

  const donationStore = useDonationStore();
  const { donations } = donationStore;

  const { toast } = useToast();

  const handleSuccess = () => {
    setShowEditOrgDialog(false);

    // show toaster
    toast({
      title: "Hurra!",
      description: "Donation is registered, add more or go to next step!",
    });
  };

  return (
    <>
      <Table>
        <TableCaption>Organisations list</TableCaption>
        <TableHeader>
          <TableRow>
            <TableHead className="">Organisajons Navn</TableHead>
            <TableHead>Offersøknad til organisajon</TableHead>
            <TableHead className="text-right">Registrert</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          {orgs.map((org) => (
            <TableRow
              key={org.id}
              className={`cursor-pointer hover:bg-slate-100 h-12 ${
                donations &&
                donations[org.id || ""] &&
                donations[org.id || ""].dates.length > 0 &&
                "bg-gray-200"
              }`}
              onClick={(event) => handleToggleDialog(org, event)}
            >
              <TableCell className="font-medium">{org.name}</TableCell>
              <TableCell>
                <Link href={org.links[1]} target="_blank">
                  Offersøknad
                </Link>
              </TableCell>

              <TableCell className="text-right">
                {donations &&
                donations[org.id || ""] &&
                donations[org.id || ""].dates.length > 0
                  ? donations[org.id || ""].dates.length
                  : 0}
              </TableCell>
            </TableRow>
          ))}
        </TableBody>
        <TableFooter>
          <TableRow></TableRow>
        </TableFooter>
      </Table>
      <GenericDialog
        isOpen={showEditOrgDialog}
        onClose={(event) => handleToggleDialog(null, event)}
      >
        {selectedOrg && (
          <RegisterDonationToOrganisationForm
            orgId={selectedOrg.id}
            onSave={handleSuccess}
          />
        )}
      </GenericDialog>
    </>
  );
}
